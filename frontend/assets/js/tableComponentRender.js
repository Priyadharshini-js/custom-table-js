...
const BUFFER_SIZE = 5;
const ROW_HEIGHT = 50; //calculate the height manualy 
...
export async function fetchData() {
    ...
    try {
        ...
        tableBodyRef.innerHTML = `<div class="no-user-found">Error fetching user data</div>`;
    } finally {
        isLoading = false;
...
export function displayBufferedUsers() {
    ...
    tableBodyRef.innerHTML = '';  // should not clearing table each time
...
function renderUserRow(item, index) {
    ...
    const columnWidths = columns.map(col => `minmax(150px, ${col.width || '1fr'})`).join(' '); //need to work on it
...
export function handleSearch(e) {
    ...
    // Dynamically get all searchable keys from column config
